...
# 一维卷积后，获得三层时域预测
class FcCvModelReC1(Model):
    def __init__(self):
        super(FcCvModelReC1, self).__init__()
        # 多层全链接的第一层，每一道数据的提取
        # 实际上是在每道上的滑动全连接
        self.fcOne = Sequential([
            layers.Dense(64),
        ...
        self.conv7 = Sequential([
            layers.Conv1D(filters=512, kernel_size=3, strides=1, padding='same'),
            layers.BatchNormalization(),
            layers.LeakyReLU(0.2)
        ])

        # 数据变成 batchsize * 3 * 256
...
# 一维卷积后，再全连接获得地层+全连接预测
class FcCvModelReFC1(Model):
    def __init__(self):
        super(FcCvModelReFC1, self).__init__()
        # 多层全链接的第一层，每一道数据的提取
        # 实际上是在每道上的滑动全连接
        self.fcOne = Sequential([
            layers.Dense(64),
        ...
        # 最终作出的预测
        # 深度预测
        self.fcThree_1 = Sequential([
            layers.Dense(256)
        ])

        self.fcThree_2 = Sequential([
            layers.Dense(256)
        ])

        self.fcThree_3 = Sequential([
            layers.Dense(256)
        ])
        # 速度预测
        self.fcThree = Sequential([
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        x = self.conv6(x)
        x = tf.reshape(x, (batchSize, -1))
        x_1 = self.fcThree_1(x)
        x_2 = self.fcThree_2(x)
        x_3 = self.fcThree_3(x)
        x_s = self.fcThree(x)
        x = tf.concat([tf.expand_dims(x_1, axis=1), tf.expand_dims(x_2, axis=1), tf.expand_dims(x_3, axis=1)], axis=1)
        return x, x_s
...
class FcCvModelReFCDepth(Model):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        x_d = tf.concat([tf.expand_dims(x_d1, axis=1), tf.expand_dims(x_d2, axis=1), tf.expand_dims(x_d3, axis=1)],
                        axis=1)
        return x_d
